...
{
    public static class TopeMedia
    {
        [FunctionName("TopMedia")]
            ...
            ILogger log)
        {
            ...
            try
            {
                result = await GetTopMedia(token, queryUser);
        ...
        private static async Task<dynamic> GetTopMedia(string token, string businessName)
        {
            string igUser = string.Empty;
            igUser = await GetUserInfo(businessName, token);
            string url = "https://graph.facebook.com/";
            string ver = "v4.0/";
            string user_id = "17841405375274306";//owner
            string urlRequest = "/top_media?fields=like_count%2Ccomments_count%2Cmedia_type,permalink,caption,media_url&user_id=";
            ...
            if (response.StatusCode == HttpStatusCode.OK)
            {
                ...
                foreach (JObject item in media["data"])
                {
                    ...
                    var type = item["media_type"].ToString();
